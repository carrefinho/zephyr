...
#else /* !(NRF54LX && !SINGLE_TIMER && !TIFS_HW) */
	if (trx) {
		nrf_timer_publish_set(EVENT_TIMER, HAL_EVENT_TIMER_TRX_EVENT,
				      HAL_RADIO_ENABLE_TX_ON_TICK_PPI);
		nrf_radio_subscribe_set(NRF_RADIO, NRF_RADIO_TASK_TXEN,
...
/* Enable same DPPI in Peripheral domain. (The PPIB11/PPIB21 bridge above
	 * routes GRTC (DPPIC20) -> NRF_TIMER10 (DPPIC10): in single-timer mode
	 * NRF_TIMER10 is the EVENT_TIMER; in dual-timer mode it is the
	 * SW_SWITCH_TIMER, and a separate PPIB20/PPIB01 bridge below handles the
	 * EVENT_TIMER = NRF_TIMER00 start.)
	 */
	...
	nrf_timer_subscribe_set(EVENT_TIMER, NRF_TIMER_TASK_START,
				HAL_NRF54LX_DUAL_TIMER_EVENT_TIMER_START_DPPIC00);
#else /* single-timer (or TIFS_HW / non-54L GRTC) */
