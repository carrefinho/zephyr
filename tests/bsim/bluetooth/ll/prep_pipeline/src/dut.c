...
/* CPU-latency injector lives in lat_inject.c so split/host can run the
 * identical injection as a single-link control (see lat_inject.h).
 */
static void test_args_parse(int argc, char *argv[])
{
	lat_inject_args_parse(argc, argv);
}
